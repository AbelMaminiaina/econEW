...
import ProductDetailClient, { type CatalogItem } from './ProductDetailClient';
...
// Catalogue allégé pour la barre latérale (compteurs par catégorie, produits en vedette)
async function getCatalog(): Promise<CatalogItem[]> {
  try {
    const res = await fetch(`${SERVER_API_BASE_URL}/products`, { cache: 'no-store' });
    if (!res.ok) return [];
    const data: { products: CatalogItem[] } = await res.json();
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const imageUrl = product.images?.[0]?.startsWith('http')
    ? product.images[0]
    : `https://all.mg${product.images?.[0] || '/electro/img/carousel-1.jpg'}`;
...
export default async function ProductDetailPage({ params }: PageProps) {
  ...
  const [product, relatedProducts, catalog] = await Promise.all([
    getProduct(slug),
