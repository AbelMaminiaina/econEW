...
export default function AdminOrdersPage() {
  ...
  const handlePrint = (order: Order) => {
    const printContent = `
      ...
      ${order.invoiceNumber ? `N° Facture: ${order.invoiceNumber}` : ''}
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-blue-800">
                        Facture {selectedOrder.invoiceNumber}
                      </span>
                    </div>
                    <span className="text-blue-700 text-sm">
                      {selectedOrder.paymentTerms ? paymentTermsLabels[selectedOrder.paymentTerms] || selectedOrder.paymentTerms : ''}
                      {selectedOrder.dueDate && ` — échéance ${new Date(selectedOrder.dueDate).toLocaleDateString('fr-FR')}`}
                    </span>
                  </div>
                </div>
              )}
