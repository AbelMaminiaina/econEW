...
export default function AdminEntreprisesPage() {
  ...
  const [paymentTerms, setPaymentTerms] = useState<'net_30' | 'net_60'>('net_30');
  ...
  const handleApprove = async () => {
    ...
    try {
      await approveCompany(
        approveModal.id,
        { paymentTerms, creditLimit: creditLimit ? Number(creditLimit) : undefined },
        token
      );
      ...
      >
        <div className="space-y-4">
          <p className="text-warm-600 text-sm">
            {approveModal && (
              <>Approuver <span className="font-medium text-warm-800">{approveModal.name}</span> et définir ses conditions de paiement.</>
            )}
          </p>
          <Select
            label="Conditions de paiement"
            value={paymentTerms}
            onChange={(e) => setPaymentTerms(e.target.value as 'net_30' | 'net_60')}
            options={[
          ...
          />
          <Input
            label="Plafond de crédit (Ar, optionnel)"
            type="number"
            value={creditLimit}
