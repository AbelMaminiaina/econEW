            ...
            >
              ...
              <div className="p-4 sm:p-6 space-y-6">
                ...
                {selectedOrder.invoiceNumber && (
                  <div className="flex items-center justify-between p-4 bg-prairie-50 rounded-xl flex-wrap gap-2">
                    <div>
                      <p className="font-medium text-warm-800">Facture {selectedOrder.invoiceNumber}</p>
                      {selectedOrder.dueDate && (
                        <p className="text-sm text-warm-600">
                          Échéance le {new Date(selectedOrder.dueDate).toLocaleDateString('fr-FR')}
                          {selectedOrder.paymentTerms && ` (${paymentTermsLabels[selectedOrder.paymentTerms] || selectedOrder.paymentTerms})`}
                        </p>
                      )}
                    </div>
                    {selectedOrder.invoiceStatus && (
                      <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-white text-prairie-700">
                        {selectedOrder.invoiceStatus === 'paid' ? 'Payée' : selectedOrder.invoiceStatus === 'overdue' ? 'En retard' : selectedOrder.invoiceStatus === 'cancelled' ? 'Annulée' : 'Envoyée'}
                      </span>
                    )}
                  </div>
                )}
